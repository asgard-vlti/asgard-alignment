...
class UController:
    """
    Generic microcontroller class that talks to something like a controllino or teensy
    """
...
class PowerControllino(UController):
    def __init__(self, ip, port=23, init_motors=True):
        super().__init__(ip, port, init_motors)
...
class RotationMotorTeensy(UController):
    ...
    Saving conops:
    1. Rotation stage gets an offline command, calls offline function on rotation class instance
    2. instance asks controller to write current steps to save file (and provides steps).
       Steps written with a timestamp. Current steps are stored by ??? the rotation class?
    3. instance disables ucontroller line
    ...
    def __init__(self, ip, port=23, savepth=None):
        ...
        self.disable_all()
        self.line_status = {n: "disabled" for n in STEPPER_NAME_TO_NUM.keys()}
    ...
    def disable_all(self):
        self.send_command_anyreply(f"d{-1}")
    ...
    def amove(self, motor: int, position: int) -> bool:
        ...
        bool
            Status of the command.
        """  # TODO: check if this still makes senses
        return self.send_command(f"s{motor} {position}")
