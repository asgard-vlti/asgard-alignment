...
adc_names = ["ADC1A", "ADC1B", "ADC2A", "ADC2B", "ADC3A", "ADC3B", "ADC4A", "ADC4B"]
...
def main():
    ...
    print(f"Calculated ADC target positions: A={adc_a_target} centidegrees, B={adc_b_target} centidegrees")
    
    # Slew the ADCs to the target positions. This is done as a
    # special relative move.
    for AB, adc_target, adc_positions in zip(["A", "B"], [adc_a_target, adc_b_target], [adc_a_positions, adc_b_positions]):
        ...
        if (adc_1_currentpos != adc_1_abs_target):
            print(f"ERROR: ADC {AB}1 did not reach the target position within 60 seconds.")
            sys.exit(1)
