...
def plot_basis_summary(basis, n_plots):
    # if n_plots is -1, plot all. Figure out a subplots grid that makes and is as square as possible
    if isinstance(basis, hcipy.ModeBasis):
        grid = basis.grid
        basis = basis.transformation_matrix
    if n_plots == -1:
        n_plots = basis.shape[1]
    ...
    clims = np.abs(basis).max() * np.array([-1, 1])
    for i in range(n_plots):
        plt.subplot(n_rows, n_cols, i + 1)
        # 0 is first, n_plots-1 is last
        idx = int(i * basis.shape[1] / n_plots)
        plt.imshow(
            basis[:, idx].reshape(grid.shape),
            cmap="RdBu",
            vmin=clims[0],
            vmax=clims[1],
        )
