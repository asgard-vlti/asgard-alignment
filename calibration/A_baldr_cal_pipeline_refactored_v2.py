...
class PlotManager:
    def __init__(self, mode: str):
        valid_modes = {"individual", "none", "summary"}
        if mode not in valid_modes:
            raise ValueError(f"Invalid plot mode '{mode}'. Expected one of {sorted(valid_modes)}")
        self.mode = mode
        self.items: List[PlotItem] = []

    def _run_item(self, item: PlotItem):
        if item.kind == "util_nice_heatmap_subplots":
            util.nice_heatmap_subplots(**item.kwargs)
        elif item.kind == "imshow_grid":
            self._plot_imshow_grid(**item.kwargs)
        elif item.kind == "strehl_registration":
            self._plot_strehl_registration(**item.kwargs)
        elif item.kind == "opd_summary":
            self._plot_opd_summary(**item.kwargs)
        elif item.kind == "scatter":
            self._plot_scatter(**item.kwargs)
        elif item.kind == "line":
            self._plot_line(**item.kwargs)
        else:
            raise ValueError(f"Unknown plot item kind: {item.kind}")
    ...
    def add(self, kind: str, **kwargs):
        if self.mode == "none":
            return
        item = PlotItem(kind=kind, kwargs=kwargs)
        if self.mode == "individual":
            self._run_item(item)
            self._show_or_close()
    ...
    def finalize(self):
        if self.mode != "summary" or not self.items:
            return
    ...
    @staticmethod
    def _plot_imshow_grid(images, title=None, savefig=None):
        n = len(images)
        if n == 0:
            return
        ncols = min(3, n)
        ...
        for ax, entry in zip(axes_flat, images):
            img = entry["img"]
            img_title = entry.get("title", "")
    ...
    @staticmethod
    def _plot_strehl_registration(data, exterior_filter, secondary_filter, title=None, savefig=None):
        label = "I0-N0"
        fs = 18
    ...
    @staticmethod
    def _plot_opd_summary(correlation_map, strehl_filt, beam_id=None):
        util.nice_heatmap_subplots(
            im_list=[correlation_map, strehl_filt],
            cbar_label_list=["Pearson R", "filt"],
            title_list=[f"beam{beam_id} corr" if beam_id is not None else "corr", f"beam{beam_id} filt" if beam_id is not None else "filt"],
        )

    @staticmethod
    def _plot_scatter(x, y, x2=None, y2=None, xlabel="", ylabel="", label1="", label2="", title=None):
        plt.figure()
    ...
    @staticmethod
    def _plot_line(y, x=None, xlabel="", ylabel="", title=None, semilogy=False, label=None, savefig=None):
        plt.figure()
        if x is None:
            x = np.arange(len(y))
        if semilogy:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--plot_mode",
        type=str,
        choices=["individual", "none", "summary"],
        default="individual",
        help="Plot handling mode: show each plot as generated, disable plots, or defer all plots to a single summary at the end.",
    )
    parser.add_argument(
        "--fig_path",
        type=str,
...
def build_runtime(args: argparse.Namespace) -> RuntimeContext:
    ...
    return RuntimeContext(
        ...
        message_history=[],
        plotter=PlotManager(args.plot_mode),
    )
...
def register_pupils(rt: RuntimeContext) -> None:
    ask_continue(
        f"\n======================\nmoving to phasemask {rt.args.phasemask} reference position, press enter to continue. enter 0 to exit\n"
    )
    ...
    for beam_id in rt.args.beam_id:
        ...
        ell1 = util.detect_pupil(
            clear_imgs[beam_id], sigma=2, threshold=0.5, plot=(rt.args.plot_mode == "individual"), savepath=savepath
        )
        ...
        if rt.args.plot_mode == "summary":
            overlay = clear_imgs[beam_id].copy()
            overlay = np.ma.array(overlay, mask=np.zeros_like(overlay, dtype=bool))
            rt.plotter.add(
                "imshow_grid",
                title=f"Pupil registration summary",
                images=[{
                    "img": clear_imgs[beam_id],
                    "title": f"beam {beam_id} clear pupil",
                }],
                savefig=None,
            )
...
def register_dm(rt: RuntimeContext) -> Dict[int, np.ndarray]:
    ...
    for beam_id in rt.args.beam_id:
        ...
        transform_dicts[beam_id] = DM_registration.calibrate_transform_between_DM_and_image(
            dm_4_corners, img_4_corners[beam_id], debug=(rt.args.plot_mode == "individual"), fig_path=beam_fig_dir + "/"
        )
        plt.close("all")
        ...
        if rt.args.plot_mode == "summary":
            summary_images.append({"img": img, "title": f"beam {beam_id} DM reg delta"})

    if summary_images:
        rt.plotter.add("imshow_grid", images=summary_images, title="DM registration summary")
...
def register_strehl_pixels(rt: RuntimeContext, dark_dict: Dict[int, np.ndarray]) -> Tuple[Dict[int, np.ndarray], Dict[int, np.ndarray], Dict[int, np.ndarray]]:
    ...
    rt.plotter.add(
        "util_nice_heatmap_subplots",
        im_list=[zwfs_pupils[b] for b in rt.args.beam_id],
        title_list=[f"beam{b} ZWFS pupil" for b in rt.args.beam_id],
        savefig="delme.png",
    )
...
def build_opd_model(rt: RuntimeContext, dark_dict: Dict[int, np.ndarray]) -> None:
    ...
    for beam_id in rt.args.beam_id:
        ...
        rt.plotter.add("opd_summary", correlation_map=correlation_map, strehl_filt=strehl_filt, beam_id=beam_id)
        ...
        rt.plotter.add(
            ...
            y=opd_nm_est,
            xlabel="<s>",
            ylabel="OPD RMS [nm RMS]",
            label1="est",
            title=f"beam {beam_id} OPD proxy",
        )
        ...
        rt.plotter.add(
            ...
            x2=x,
            y2=opd_pred,
            xlabel="signal (i_ext / <N0>)",
            ylabel="OPD [nm RMS]",
            label1="meas",
            label2="pred",
            title=f"beam {beam_id} OPD fit",
        )
...
def build_im(rt: RuntimeContext, dark_dict: Dict[int, np.ndarray], Cn_dict: Dict[int, np.ndarray]) -> None:
    ...
    for beam_id in rt.args.beam_id:
        ...
        singular_value_data.append((beam_id, S))
        savefig = os.path.join(rt.args.fig_path, f"IM_singularvalues_beam{beam_id}.png")
        rt.plotter.add(
            "line",
            y=S,
            xlabel="mode index",
            ylabel="singular values",
            title=f"beam {beam_id} IM singular values",
            semilogy=True,
            savefig=savefig,
        )
