...
default_toml = os.path.join(
    "/usr/local/etc/baldr/", "baldr_config_#.toml"
)  # os.path.dirname(os.path.abspath(__file__)), "..", "config_files", "baldr_config.toml")
parser.add_argument(
    "--toml_file",
    type=str,
...
parser.add_argument(
    "--host", type=str, default="192.168.100.2", help="Server host"
)  # "localhost"
...
for beam_id in args.beam_id:
    ...
    Cn_dict[beam_id] = np.cov(
        Dtmp.T
    )  # (Xtmp.T @ Xtmp) / (Xtmp.shape[0] - 1)        # (P, P) unbiased sample covariance
...
util.nice_heatmap_subplots(
    im_list=[dark_dict[beam_id] for beam_id in args.beam_id],
    title_list=[f"beam{beam_id} dark" for beam_id in args.beam_id],
)
plt.show()

util.nice_heatmap_subplots(
    im_list=[Cn_dict[beam_id] for beam_id in args.beam_id],
    title_list=[f"beam{beam_id} dark covariance" for beam_id in args.beam_id],
)
plt.show()
...
for beam_id in args.beam_id:
    ...
    ell1 = util.detect_pupil(
        cropped_img, sigma=2, threshold=0.5, plot=args.plot, savepath=savepath
    )
    ...
    exterior = util.filter_exterior_annulus(
        pupil_mask, inner_radius=rad_est + 1, outer_radius=rad_est + 5
    )
...
dm_4_corners = DM_registration.get_inner_square_indices(
    outer_size=12, inner_offset=4
)  # flattened index of the DM actuator
...
for beam_id in args.beam_id:
    with open(args.toml_file.replace("#", f"{beam_id}")) as file:
        ...
        pupil_mask[beam_id] = (
            config_dict.get(f"beam{beam_id}", {})
            .get("pupil_mask", {})
            .get("mask", None)
        )
    ...
    for (
        act
    ) in dm_4_corners:  # 4 corner indicies are in 140 length vector (not 144 2D map)
        print(f"actuator {act}")
        img_list_push = [[] for _ in args.beam_id]
        img_list_pull = [[] for _ in args.beam_id]
        poke_vector = np.zeros(
            140
        )  # 4 corner indicies are in 140 length vector (not 144 2D map)
        for nn in range(number_of_pokes):
            ...
            for ii, beam_id in enumerate(args.beam_id):
                dm_shm_dict[beam_id].set_data(
                    dm_shm_dict[beam_id].cmd_2_map2D(poke_vector, fill=0)
                )
                ...
                # zero the poke
                dm_shm_dict[beam_id].set_data(
                    dm_shm_dict[beam_id].cmd_2_map2D(0 * poke_vector, fill=0)
                )
        ...
        for ii, beam_id in enumerate(args.beam_id):
            delta_img = abs(
                np.mean(img_list_push[ii], axis=0) - np.mean(img_list_pull[ii], axis=0)
            )
            # the mean difference in images from push/pulls on the current actuator
            img_4_corners[ii].append(
                np.array(pupil_mask[beam_id]).astype(float) * delta_img
            )  #  We multiply by the pupil mask to ignore all external pixels! These can be troublesome with hot pixels etc
...
for ii, beam_id in enumerate(args.beam_id):
    ...
    transform_dicts.append(
        DM_registration.calibrate_transform_between_DM_and_image(
            dm_4_corners,
            img_4_corners[ii],
            debug=True,
            fig_path=savefig + f"beam{beam_id}/",
        )
    )
    plt.close()  # close any open figiures
    ...
    x_target = np.array(
        [x for x, _ in transform_dicts[ii]["actuator_coord_list_pixel_space"]]
    )
    y_target = np.array(
        [y for _, y in transform_dicts[ii]["actuator_coord_list_pixel_space"]]
    )
...
def plot_strehl_pixel_registration(
    data, exterior_filter, secondary_filter, savefig=None
):
    ...
    if np.sum(exterior_filter):
        ...
        ext_x_min, ext_x_max = 0.5 + np.min(
            np.where(np.abs(np.diff(exterior_filter, axis=1)) > 0)[1]
        ), 0.5 + np.max(np.where(np.abs(np.diff(exterior_filter, axis=1)) > 0)[1])
        ext_y_min, ext_y_max = 0.5 + np.min(
            np.where(np.abs(np.diff(exterior_filter, axis=0)) > 0)[0]
        ), 0.5 + np.max(np.where(np.abs(np.diff(exterior_filter, axis=0)) > 0)[0])

    if np.sum(secondary_filter):
        # Secondary filter boundaries (blue)
        sec_x_min, sec_x_max = 0.5 + np.min(
            np.where(abs(np.diff(secondary_filter, axis=1)) > 0)[1]
        ), 0.5 + np.max(np.where(abs(np.diff(secondary_filter, axis=1)) > 0)[1])
        sec_y_min, sec_y_max = 0.5 + np.min(
            np.where(abs(np.diff(secondary_filter, axis=0)) > 0)[0]
    ...
    if np.sum(exterior_filter):
        ...
        ax_main.scatter(
            ...
            color="red",
            alpha=0.4,
            label="Exterior Filter",
        )

    if np.sum(secondary_filter):
        ax_main.contour(
            secondary_filter.astype(float),
            levels=[0.5],
            extent=[-0.5, data.shape[1] - 0.5, -0.5, data.shape[0] - 0.5],
            colors="blue",
            linestyles="-",
...
for beam_id in args.beam_id:
    ...
    exterior_filter = (
        (
            abs(zwfs_pupils[beam_id] - clear_pupils[beam_id])
            > args.lobe_threshold * np.mean(clear_pupils[beam_id][pupil_mask])
        )
        * (~pupil_mask)
        * pupil_edge_filter
        * pupil_limit_filter
...
for _ in range(number_of_screen_initiations):
    ...
    dm_scrn = ps.PhaseScreenKolmogorov(
        nx_size=24, pixel_scale=1.8 / 24, r0=r0, L0=L0, random_seed=None
    )
...
for beam_id in args.beam_id:
    ...
    with open(args.toml_file.replace("#", f"{beam_id}"), "r") as f:
        ...
        inner_pupil_filt = np.array(
            config_dict.get(f"beam{beam_id}", {})
            .get(f"{args.phasemask}", {})
            .get("ctrl_model", None)
            .get("inner_pupil_filt", None)
        )  # .astype(bool)
        # clear pupil
        N0 = np.array(
            config_dict.get(f"beam{beam_id}", {})
        ...
        N0_norm = np.mean(
            N0[inner_pupil_filt.astype(bool)]
        )  # defined in build_IM_in_subframe (interaction matrix standard)
    ...
    for it in range(len(scrn_list)):
        ...
        for ph_scale in scrn_scaling_grid:
            ...
            cmd = util.create_phase_screen_cmd_for_DM(
                scrn_list[it],
    ...
    correlation_map = util.compute_correlation_map(
        np.array(telem["s"]), np.array(telem["opd_rms_est"])
    )

    yy, xx = np.ogrid[: telem["s"][0].shape[0], : telem["s"][0].shape[0]]
    snr = np.mean(np.array(telem["s"]), axis=0) / np.std(np.array(telem["s"]), axis=0)
    radial_constraint = (
        (xx - telem["s"][0].shape[0] // 2) ** 2
        + (yy - telem["s"][0].shape[0] // 2) ** 2
        <= 20**2
    ) * (
        (xx - telem["s"][0].shape[0] // 2) ** 2
        + (yy - telem["s"][0].shape[0] // 2) ** 2
    ...
    util.nice_heatmap_subplots(
        im_list=[correlation_map, strehl_filt], cbar_label_list=["Pearson R", "filt"]
    )
    # savefig = save_results_path + 'strehl_vs_intensity_pearson_R.png' ) #fig_path + 'strehl_vs_intensity_pearson_R.png' )

    plt.figure()
    plt.plot(
        [np.mean(ss[strehl_filt]) for ss in telem["s"]],
        np.array(telem["opd_rms_est"]),
        ".",
        label="est",
    )
    ...
    # could put some mroe safety here
    opd_model_params = util.fit_piecewise_continuous(
        x=filtered_sigs, y=opd_nm_est, n_grid=80, trim=0.15
    )

    print(
        f"using util.fit_piecewise_continuous, opd_model_params = {opd_model_params} \ninput these to util.fit_piecewise_continuous"
    )
    ...
    dict2write = {
        f"beam{beam_id}": {
            f"{args.phasemask}": {
                "ctrl_model": {
                    "strehl_filter": np.array(strehl_filt)
                    .astype(int)
                    .reshape(-1)
                    .tolist(),
...
for beam_id in args.beam_id:
    ...
    with open(args.toml_file.replace("#", f"{beam_id}"), "r") as f:
        ...
        secondary_mask[beam_id] = np.array(
            config_dict.get(f"beam{beam_id}", {})
            .get("pupil_mask", {})
            .get("secondary", None)
        )

        exterior_mask[beam_id] = np.array(
            config_dict.get(f"beam{beam_id}", {})
            .get("pupil_mask", {})
            .get("exterior", None)
        )
...
for beam_id in args.beam_id:
    ...
    inner_pupil_filt[beam_id] = binary_erosion(
        pupil_mask[beam_id] * (~secondary_mask[beam_id].astype(bool)),
    ...
    pixel_filter = secondary_mask[beam_id].astype(bool) | (
        ~(util.remove_boundary(np.array(pupil_mask[beam_id])).astype(bool))
    )  # | (~bad_pix_mask_tmp )
    normalized_pupils[beam_id] = np.mean(clear_pupils[beam_id], axis=0)
    normalized_pupils[beam_id][pixel_filter] = np.mean(
        np.mean(clear_pupils[beam_id], 0)[~pixel_filter]
    )  # set exterior and boundary pupils to interior mean
...
if "zonal" in args.basis_name.lower().strip():
    ...
    raise UserWarning(
        f'invalid --basis_name={args.basis_name} input. must be "zonal" or "zernike",'
    )
...
# we now always build IM in measurement space, and consider signal space when building reconstructor in 'build_baldr_control_matrix.py'
...
# Preallocate storage
Iplus_stack = {
    b: np.zeros((n_modes, n_plus, ny, nx), dtype=np.float32) for b in args.beam_id
}
Iminus_stack = {
    b: np.zeros((n_modes, n_minus, ny, nx), dtype=np.float32) for b in args.beam_id
}
...
for b in args.beam_id:
    ...
    # masks
    hdul.append(
        fits.ImageHDU(np.asarray(pupil_mask[b], dtype=np.uint8), name=f"PUPIL_B{b}")
    )
    hdul.append(
        fits.ImageHDU(np.asarray(secondary_mask[b], dtype=np.uint8), name=f"SEC_B{b}")
    )
    hdul.append(
        fits.ImageHDU(np.asarray(exterior_mask[b], dtype=np.uint8), name=f"EXT_B{b}")
    )
    hdul.append(
        fits.ImageHDU(
            np.asarray(inner_pupil_filt[b], dtype=np.uint8), name=f"INNER_B{b}"
        )
    )
    ...
    hdul.append(
        fits.CompImageHDU(
            np.asarray(Iplus_stack[b], dtype=np.float32),
            name=f"IPLUS_B{b}",
    ...
    hdul.append(
        fits.CompImageHDU(
            np.asarray(Iminus_stack[b], dtype=np.float32),
            name=f"IMINUS_B{b}",
            compression_type="RICE_1",
        )
    )
    hdul.append(
        fits.CompImageHDU(
            np.asarray(IM_mat[b], dtype=np.float32),
            name=f"IM_FINAL",
            compression_type="RICE_1",
        )
    )
            ...
            f"{args.phasemask}": {
                "ctrl_model": {
                    ...
                    "crop_pixels": np.array(
                        baldr_pupils[f"{beam_id}"]
                    ).tolist(),  # global corners (r1,r2,c1,c2) of sub pupil cropping region  (local frame)
                    "pupil_pixels": np.where(np.array(pupil_mask[beam_id]).reshape(-1))[
                        0
                    ].tolist(),  # pupil pixels in local frame
                    "interior_pixels": np.where(
                        np.array(inner_pupil_filt[beam_id].reshape(-1))
                    )[
                        0
                    ].tolist(),  # strictly interior pupil pixels in local frame
                    "secondary_pixels": np.where(
                        np.array(secondary_mask[beam_id].reshape(-1))
                    )[
                        0
                    ].tolist(),  # pixels in secondary obstruction in local frame
                    "exterior_pixels": np.where(
                        np.array(exterior_mask[beam_id].reshape(-1))
                    )[
                        0
                    ].tolist(),  # exterior pixels that maximise diffracted light from mask in local frame
                    # "bad_pixels" : np.where( np.array( c.reduction_dict['bad_pixel_mask'][-1])[r1:r2,c1:c2].reshape(-1)   )[0].tolist(),
                    "IM": np.array(
                        IM_mat[beam_id]
                    ).tolist(),  # np.array( IM[beam_id] ).tolist(),
                    ...
                    .reshape(-1)
                    .tolist(),  ## ## post TTonsky  #(float( c.config["fps"] ) / float( c.config["gain"] ) * np.mean( zwfs_pupils[beam_id],axis=0).reshape(-1) ).tolist(),  # ADU / s / gain (flattened)
                    "intrn_flx_I0": float(
                        np.sum(np.mean(zwfs_pupils[beam_id], axis=0))
                    ),
                    "N0": np.mean(clear_pupils[beam_id], axis=0)
                    .reshape(-1)
                    .tolist(),  ## ## post TTonsky #(float( c.config["fps"] ) / float( c.config["gain"] ) * np.mean( clear_pupils[beam_id],axis=0).reshape(-1) ).tolist(), # ADU / s / gain (flattened)
                    "norm_pupil": np.array(normalized_pupils[beam_id])
                    .reshape(-1)
                    .tolist(),  ## post TTonsky #( float( c.config["fps"] ) / float( c.config["gain"] ) * np.array( normalized_pupils[beam_id] ).reshape(-1) ).tolist(),
                    ...
                    "pupil": np.array(pupil_mask[beam_id])
                    .astype(int)
                    .reshape(-1)
                    .tolist(),
                    "secondary": np.array(secondary_mask[beam_id])
                    .astype(int)
                    .reshape(-1)
                    .tolist(),
                    "exterior": np.array(exterior_mask[beam_id])
                    .astype(int)
                    .reshape(-1)
                    .tolist(),
                    "inner_pupil_filt": np.array(inner_pupil_filt[beam_id])
                    .astype(int)
                    .reshape(-1)
                    .tolist(),
                    ...
                    "dark": np.array(dark_dict[beam_id])
                    .astype(int)
                    .reshape(-1)
                    .tolist(),  # np.zeros([32,32]).reshape(-1).astype(int).tolist(), # just update to a default 1000 adu offset. In rtc this can be updated with dark_update function!
                    "dark_cov": np.array(Cn_dict[beam_id])
                    .astype(int)
                    .reshape(-1)
                    .tolist(),
                    "bad_pixel_mask": np.ones([32, 32])
                    .reshape(-1)
                    .astype(int)
                    .tolist(),
                    "bad_pixels": [],
...
for beam_id in args.beam_id:
    ...
    plt.savefig(
        f"{args.fig_path}" + f"IM_singularvalues_beam{beam_id}.png",
        bbox_inches="tight",
        dpi=200,
    )
    plt.show()
